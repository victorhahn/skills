...
If `pnpm` is missing, install it (`corepack enable && corepack prepare pnpm@latest --activate`) rather than fall back to `npm`. The user's global guidance is pnpm-first.
