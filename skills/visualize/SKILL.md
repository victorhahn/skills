...
name: visualize
description: >
  Invoke when the answer should be a diagram, not prose. Three triggers —
  (1) explicit request — "visualize", "diagram this", "draw the flow", "show me
  the relationships", "I can't picture it", "help me see the difference";
  (2) sharing intent — user wants something to send to a teammate, drop in
  Slack, put on a wiki, or present to a PM or stakeholder;
  (3) structural complexity — user describes a multi-entity permission model,
  event-driven pipeline, deployment sequence, auth flow, or architectural
  tradeoff and needs to understand or communicate how pieces connect.
  This is the only skill that produces rendered Mermaid diagrams and
  self-contained HTML visuals. Skip for direct code writing, SQL queries,
  factual lookups, and debugging investigations.
---
