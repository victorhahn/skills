...
2. For each: make the change, run the relevant tests/typecheck/linter, commit with a conventional-commit message describing the *why*. Use the user's conventional commit style (`fix:`, `chore:`, `refactor:`, `test:` — see CLAUDE.md if present).
3. If any check fails: stop, surface the failure, do not auto-fix and continue.
4. Do not push or open PRs unless the user explicitly authorizes it (hard stop per global CLAUDE.md).
...
Match the user's communication style from CLAUDE.md if present — terse, no filler. Bullets, tables, bold key terms. Prose only for headline and rollout-plan narrative.
