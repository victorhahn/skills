...
- Behavior-driven / spec-driven tests over implementation-coupled tests (matches the global CLAUDE.md TDD philosophy).
...
- The user's global CLAUDE.md explicitly states: "Prefer end-to-end type safety. Generate types from schemas (OpenAPI, Zod, Protobuf) rather than maintaining parallel hand-written definitions. Prefer type-safe generated clients over hand-rolled fetch wrappers."
...
- **Std-lib replacements for third-party deps:** native `fetch` replacing `axios`/`node-fetch`, native `crypto` for simple cases, `Intl` for date/number formatting, etc. (Matches global CLAUDE.md preference for built-in over third-party where capability is equivalent.)
...
**Look for:**
- **Profile data signals:** if Datadog APM, flamegraphs, or other profile data is available, use it. Query Datadog spans/metrics via MCP if relevant (per global CLAUDE.md). Don't speculate; query.
...
- Demands for more comments. Per global CLAUDE.md, comments are minimal by preference; only flag when accuracy or ground-truth is at stake.
...
**Modern reference points:**
- Contract-first development (matches the global preference).
